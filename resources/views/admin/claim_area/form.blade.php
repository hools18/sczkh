    ...
    <div class="modal-body">
        <div class="box-body">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" name="category_claim_id" value="{{ $claim->getCategoryName()}}" readonly>
            </div>
            <div class="form-group">
                <label>Город</label>
                <select name="worker_id" class="form-control select_worker" required>
                    <option disabled selected>Выберите исполнителя</option>
                    @foreach($workers as $worker)
                        <option value="{{ $worker->id }}" {{ $worker->id == $claim->worker_id ? 'selected' : '' }}>{{ $worker->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" name="place_description" value="{{ $claim->place_description }}" readonly>
            </div>
            ...
            @endif
            <input type="hidden" name="status" value="Передано исполнителю">
        </div>
    </div>
    <div class="modal-footer">
        <button type="button" class="btn btn-default pull-left" data-dismiss="modal">отмена</button>
        <button type="submit" class="btn btn-primary">Передать на исполнение</button>
    </div>
</form>
